"""
Update the Export To Word tool to single-file mode
"""
import sqlite3
import json
...
def export_to_word(report_markdown: str, file_name: Optional[str] = None) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "status": "ok",
            "mode": "appended" if file_existed else "created"
        }
    ...
    if isinstance(obj, dict):
        if obj.get('component_type') == 'tool' and obj.get('label') == 'Export To Word':
            ...
            if 'config' in obj and 'source_code' in obj['config']:
                ...
                obj['config']['description'] = "Export report to a single consolidated Word file"
                modified = True
...
def main():
    ...
    print("=== UPDATING EXPORT TOOL TO SINGLE-FILE MODE ===")
    ...
    print(f"   Rafeeq_Complete_Report.docx in the Script folder")
