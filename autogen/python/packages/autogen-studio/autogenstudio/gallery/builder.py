...
def create_default_gallery() -> GalleryConfig:
    ...
    base_model = OpenAIChatCompletionClient(model=default_model)
    builder.add_model(base_model.dump_component(), label=f"OpenAI {default_model}", description=f"OpenAI {default_model}")

    # Create GPT-5.1 model
    gpt5_model = OpenAIChatCompletionClient(
        model="gpt-5.1",
        model_info={
            "vision": True,
    ...
    builder.add_model(
        gpt5_model.dump_component(),
        label="OpenAI GPT-5.1",
        description="OpenAI GPT-5.1 model client.",
    )
