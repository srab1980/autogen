...
class WebSocketManager:
    ...
    async def start_stream(
        ...
        with RunContext.populate_context(run_id=run_id):
            ...
            try:
                ...
                # Update team config with current settings if available
                updated_team_config = await self._update_team_config_with_settings(team_config, user_settings)
    ...
    async def _update_team_config_with_settings(
        self, 
        team_config: str | Path | Dict[str, Any] | ComponentModel,
        user_settings: Optional[Settings]
    ) -> Dict[str, Any] | ComponentModel:
        """Update team configuration with current user settings"""
        # Convert team_config to dict if it's not already
        if isinstance(team_config, ComponentModel):
            config_dict = team_config.model_dump()
        elif isinstance(team_config, (str, Path)):
            # If it's a path, we need to load it first
            team_manager = TeamManager()
            config_dict = await team_manager.load_from_file(team_config)
        else:
            config_dict = team_config
            
        # If we have user settings, update the team config
        if user_settings and user_settings.config:
            settings_config = SettingsConfig(**user_settings.config)
            
            # If there's a default model client in settings, update team participants
            if settings_config.default_model_client:
                self._update_participants_with_model(config_dict, settings_config.default_model_client)
                
        return config_dict
        
    def _update_participants_with_model(self, config_dict: Dict[str, Any], default_model_client: Dict[str, Any]) -> None:
        """Update participant model clients with the default model from settings"""
        # Navigate to participants in the team config
        if "participants" in config_dict:
            for participant in config_dict["participants"]:
                if isinstance(participant, dict) and "model_client" in participant:
                    # Update the model client with the one from settings
                    participant["model_client"] = default_model_client
        # Handle nested structures
        elif "config" in config_dict and isinstance(config_dict["config"], dict):
            self._update_participants_with_model(config_dict["config"], default_model_client)
