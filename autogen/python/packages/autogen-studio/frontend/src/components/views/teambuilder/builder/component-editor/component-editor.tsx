...
export const ComponentEditor: React.FC<ComponentEditorProps> = ({
  ...
  const handleTestComponent = async () => {
    ...
    try {
      // Sanitize component config before testing
      const sanitizedComponent = { ...currentComponent };
      if (
        isAgentComponent(sanitizedComponent) &&
        isAssistantAgent(sanitizedComponent) &&
        Array.isArray(sanitizedComponent.config.workbench) &&
        sanitizedComponent.config.workbench.length === 0
      ) {
        // @ts-ignore
        sanitizedComponent.config.workbench = null;
      }

      const result = await validationAPI.testComponent(sanitizedComponent);
      setTestResult(result);

      if (result.status) {
        messageApi.success("Component test passed!");
      } else {
        messageApi.error(`Component test failed: ${result.message}`);
      }
    } catch (error) {
            ...
            <Button
              ...
              icon={
                <div className="relative">
                  ...
                  {testResult && (
                    <div
                      className={`absolute top-0 right-0 w-2 h-2 ${testResult.status ? "bg-green-500" : "bg-red-500"
                        } rounded-full`}
                    ></div>
                  )}
                </div>
