...
export const SessionManager: React.FC = () => {
  ...
  // Sync sidebar state with storage on client mount
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = safeStorage.getItem("sessionSidebar");
      if (stored !== null) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      safeStorage.setItem("playgroundSidebar", JSON.stringify(isSidebarOpen));
    }
  }, [isSidebarOpen]);
  ...
  return (
    <div className="relative flex h-full w-full">
      ...
      <div
        className={`flex-1 transition-all duration-200 ${isSidebarOpen ? "ml-64" : "ml-12"
          }`}
      >
