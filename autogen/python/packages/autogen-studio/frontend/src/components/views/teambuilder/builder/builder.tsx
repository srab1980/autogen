...
export const TeamBuilder: React.FC<TeamBuilderProps> = ({
  ...
  const handleImport = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Load the imported config into the builder
        const { nodes: importedNodes, edges: importedEdges } = loadFromJson(
          config,
          false, // Don't reset history - we want to mark as dirty
          team?.id?.toString()
        );
        setNodes(importedNodes);
        setEdges(importedEdges);

        // Add to history so user can undo/save
        useTeamBuilderStore.getState().addToHistory();

        messageApi.success("Team configuration imported successfully");
        handleValidate();
      } catch (error) {
        console.error("Import error:", error);
        messageApi.error("Failed to import: Invalid JSON file");
      }
    };
  ...
  }, [loadFromJson, setNodes, setEdges, team?.id, messageApi, handleValidate]);
            ...
            <Button
              ...
              icon={
                <div className="relative">
                  ...
                  {validationResults && (
                    <div
                      className={` ${teamValidated ? "bg-green-500" : "bg-red-500"
        ...
        <Layout className=" relative bg-primary  h-[calc(100vh-239px)] rounded">
          ...
          <Layout className="bg-primary rounded">
            <Content className="relative rounded bg-tertiary  ">
              <div
                className={`w-full h-full transition-all duration-200 ${isFullscreen
