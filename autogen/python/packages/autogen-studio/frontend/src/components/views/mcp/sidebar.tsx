...
export const McpSidebar: React.FC<McpSidebarProps> = ({
  ...
  const handleGallerySelect = useCallback((gallery: Gallery) => {
    ...
    if (gallery.id) {
      safeStorage.setItem("mcp-view-gallery", gallery.id.toString());
    }
  ...
  const loadGalleries = useCallback(async () => {
    ...
    try {
      ...
      const savedGalleryId = safeStorage.getItem("mcp-view-gallery");
        ...
        {mcpWorkbenches.map((workbench, index) => {
          ...
          return (
            <div key={workbenchKey} className="relative border-secondary">
              <div
                className={`absolute top-1 left-0.5 z-50 h-[calc(100%-8px)] w-1 bg-opacity-80 rounded ${isSelected ? "bg-accent" : "bg-tertiary"
                  }`}
              />
              <div
                className={`group ml-1 flex flex-col p-3 rounded-l cursor-pointer hover:bg-secondary ${isSelected
