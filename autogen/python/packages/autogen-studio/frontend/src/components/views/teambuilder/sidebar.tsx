...
export const TeamSidebar: React.FC<TeamSidebarProps> = ({
  ...
  const createTeam = () => {
    ...
    const stored = safeStorage.getItem("teamSidebar");
  ...
  return (
    <div className="h-full border-r border-secondary">
      ...
      <div className="flex border-b border-secondary">
        <button
          style={{ width: "110px" }}
          className={`flex items-center  px-2 py-1 text-sm font-medium ${activeTab === "recent"
        ...
        <button
          className={`flex items-center px-4 py-2 text-sm font-medium ${activeTab === "gallery"
            ? "text-accent border-b-2 border-accent"
            : "text-secondary hover:text-primary"
            }`}
                  ...
                  <div key={team.id} className="relative border-secondary">
                    <div
                      className={`absolute top-1 left-0.5 z-50 h-[calc(100%-8px)]
                        w-1 bg-opacity-80 rounded ${currentTeam?.id === team.id
                          ? "bg-accent"
                          : "bg-tertiary"
                        }`}
                    />
                    <div
                      className={`group ml-1 flex flex-col p-3 rounded-l cursor-pointer hover:bg-secondary ${currentTeam?.id === team.id
