...
export const ComponentLibrary: React.FC<LibraryProps> = ({
  ...
  const sections = React.useMemo(
    () => {
      const injectOrigin = (comp: any, type: ComponentTypes) => {
        return {
          ...comp,
          metadata: {
            ...comp.metadata,
            _origin: JSON.stringify({
              gallery_id: defaultGallery.id,
              component_type: type,
              component_id: comp.id || comp.client_id, // Prefer ID, fallback to client_id
            }),
          },
        };
      };
      ...
      return [
        {
          ...
          items: defaultGallery.config.components.agents.map((agent) => ({
            label: agent.label,
            config: injectOrigin(agent, "agent"),
          })),
        ...
        },
        {
          ...
          items: defaultGallery.config.components.models.map((model) => ({
            ...
            config: injectOrigin(model, "model"),
          })),
        ...
        },
        {
          ...
          items:
            defaultGallery.config.components.workbenches?.map((workbench) => ({
              label: workbench.label || workbench.provider.split(".").pop(),
              config: injectOrigin(workbench, "workbench"),
            })) || [],
        ...
        },
        {
          ...
          items: defaultGallery.config.components.terminations.map(
            (termination) => ({
              ...
              config: injectOrigin(termination, "termination"),
            })
          ),
