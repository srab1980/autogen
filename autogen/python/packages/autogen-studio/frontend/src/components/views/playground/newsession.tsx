...
const NewSessionControls = ({
  ...
  // Load last used team from storage on client side only
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = safeStorage.getItem("lastUsedTeamId");
      if (stored) {
        setLastUsedTeamId(parseInt(stored));
      }
    }
  }, []);
  ...
  const handleStartSession = async () => {
    ...
    if (typeof window !== "undefined") {
      safeStorage.setItem("lastUsedTeamId", selectedTeamId.toString());
    }
